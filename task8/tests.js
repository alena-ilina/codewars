...
describe('Выбираем обед', () => {
  ...
  it('Если хотя бы одно из полей не нужного типа, то выводим строку "Ничего не понимаю. Кажется, сбой в программе"', () => {
    chai.expect(chooseLaunch('23', 10, 'healthy')).to.equal('Ничего не понимаю. Кажется, сбой в программе')
    chai.expect(chooseLaunch(3, '10', 'healthy')).to.equal('Ничего не понимаю. Кажется, сбой в программе')
  ...
  it('Если количество рабочих задач и время после завтрака не целые числа, то выводим строку "Количество рабочих задач и время после завтрака должны быть натуральными числами"', () => {
    chai.expect(chooseLaunch(2.2, 10, 'healthy')).to.equal('Количество рабочих задач и время после завтрака должны быть натуральными числами')
    chai.expect(chooseLaunch(2, 10.5, 'healthy')).to.equal('Количество рабочих задач и время после завтрака должны быть натуральными числами')
  });
  ...
  it('Если в аппетит не передали одно из следующих значений: poor, healthy, brutal, то выводим строку "Прости, не знаю такого аппетита :("', () => {
    chai.expect(chooseLaunch(3, 3, 'hungry as a bear')).to.equal('Прости, не знаю такого аппетита')
  });
  ...
  it('Если во время после завтрака передать значение ниже 0 или больше 24, то выводим строку "Ты что! В сутках всего 24 часа!"', () => {
    ...
    chai.expect(chooseLaunch(-1, 3, 'healthy')).to.equal('Ты что! В сутках всего 24 часа!')
  });
  ...
  it('Если не сделал ни одной задачи + завтракал от 2 до 4 часов назад, то выводим строку "Извини, тебе положено только кофе"', () => {
    chai.expect(chooseLaunch(3, 0, 'poor')).to.equal('Извини, тебе положено только кофе')
  });
